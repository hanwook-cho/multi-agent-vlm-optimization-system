...
This project **uses, and does not redistribute, the third-party model weights, dataset
images, or benchmark data** below. Weights, GGUFs, and dataset photos are fetched by the
user from their original sources at run time (see `.gitignore` — they are never committed).

Two narrow exceptions are committed, both permitted:
- A small **COCO-derived** Stage-A eval set (`datasets/stage_a/`) that includes COCO
  ground-truth captions — redistributed under **CC BY 4.0** with attribution to COCO (below).
  No COCO images are committed.
- The **vendored VLMEvalKit** source under `vendor/VLMEvalKit/` (Apache-2.0; its `LICENSE`
  and a `CHANGES.md` noting our patches are preserved).

Outputs generated by the restrictively-licensed reference models (FastVLM, MiniCPM-V) are
**not** committed — only their aggregate scores/metrics are retained.
