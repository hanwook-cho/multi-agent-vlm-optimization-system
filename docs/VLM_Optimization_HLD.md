...
**P6. Training is expensive and asymmetric.** Fine-tuning a 450M VLM takes hours on M5 Pro 32 GB. Training-required improvements ("try fine-tuning with this new technique") cost much more than training-free improvements ("try this quantization scheme on existing weights"). The search must distinguish them.
...
- **Compute Mac** (M5 Pro 32 GB): runs the Experiment Runner, Evaluation Harness, training jobs, reference inference. This is where MPS bandwidth matters.
