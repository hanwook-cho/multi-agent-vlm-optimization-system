...
2. **A balanced mixture of matched distributions gives one student multiple skills — but robustness varies by axis** *(2026-06-16/17, spec `151cf686`).* A 50/50 COCO-grounding + ScienceQA student can clear **both POPE and MMBench** above floor at once. A 3-seed variance test then showed the two axes differ sharply: **MMBench is robust** (all seeds above floor, 0.59–0.70 — distribution-matching reliably delivers the targeted skill), while **POPE is high-variance** (balanced-accuracy 5–62 across identical configs; only ~half of inits above floor). So the "both floors at once" result is real but **variance-limited** — the matched-distribution skill reproduces; holding the second skill simultaneously is a coin-flip at 0.5B. **Methodological consequence:** construction claims (especially POPE) require **multi-seed reporting (median/min over ≥3 seeds)**, not single runs — single-run milestones are not trustworthy. See `docs/observations/2026-06-17-p2b1-variance-seeding.md`.
