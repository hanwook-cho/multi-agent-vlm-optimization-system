...
- **P2-B1 60/40 grounding-heavy, 3-seed (2026-06-18) — partial; ratio shifts POPE's mean, not its variance.** Going 50/50→60/40 lifted POPE bal-acc median 13.3→**48.3** (min 5→30) — more grounding clearly helps POPE (and confirms the earlier unseeded "60/40→13" was a bad draw). But POPE is **still only 1/3 seeds above floor** (median just under 50) with a ~40-pt spread, and MMBench dropped to 0.55–0.58 (less ScienceQA, still all above floor). **The variance — not the ratio — is the wall:** POPE's spread stays wide regardless of mix → implicates **capacity/training-stability at 0.5B**, not data balance. No mix yet gives one student reliably above ≥2 floors across seeds; the Goals multi-skill bar is gated on variance/capacity, not data. Fork: push ratio further (data-only, limited); attack variance (bigger LM — breaks edge budget; or projector warm-start / batch>1); or ship per-axis single-skill students. See [observation](docs/observations/2026-06-18-p2b1-6040-multiseed.md).
- **P2-B1 CONSOLIDATED (2026-06-18) — status → CONFIRMED, paused.** Decision: consolidate the honest result rather than grind further at 0.5B. **Confirmed:** the system-driven construction loop works end-to-end, and **distribution matching is the dominant lever** (COCO→POPE, ScienceQA→MMBench; MMBench reproduces across seeds). Per-axis deliverables: `d3423bc0` (POPE grounding 68.3) and `b2feb6b1` (MMBench ~0.65). **Deferred to capacity/Phase-3:** a single student *robustly* clearing ≥2 floors — variance-limited at 0.5B (ratio shifts POPE's mean, not its variance). The Goals multi-skill bar is gated on capacity, not data. Methodology locked in: floor-adjusted eval + multi-seed reporting + verify-before-building. Full arc: [P2-B1 retrospective](docs/observations/2026-06-18-p2b1-retrospective.md). Reopen with a larger LM (Phase-3) or a training-stability fix.
- **On-device validation via MLX (2026-06-18, ADR-0014) — ✅ edge half closed for `d3423bc0`.** Ported the constructed student (SigLIP + projector + Qwen2.5-0.5B + LoRA) to MLX in Python and verified it reproduces the PyTorch student: faithful MLX SigLIP (last_hidden_state max|Δ|=2e-04), **assembled-forward parity gate PASSED — 5/5 images produce identical multi-token captions** (strengthened from an initial single yes/no; a coincidental multi-image match is effectively impossible). Apple-Silicon (M4 MLX) perf: **TTFT ~115 ms, ~80 tok/s, peak ≈1.6 GB** — fits the iPhone 16 Pro budget (vs Qwen2.5-VL-3B's non-viable 6.5 GB). This closes the *edge-deployability* half of the proof-of-work (quality was already validated same-path). On-device **bundle** built (~1.14 GB self-contained MLX); iPhone **Swift port deferred** — `mlx-swift`'s `Qwen2Model` exposes no `inputsEmbeds` path (the student is LLaVA-style/prepend), so it needs a vendored custom decoder, and the verified Mac-MLX number makes it non-load-bearing. Tool: `runners/export_student_mlx.py`. See [observation](docs/observations/2026-06-18-on-device-mlx-validation.md).
