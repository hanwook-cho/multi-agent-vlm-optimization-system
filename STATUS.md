...
- **On-device validation via MLX (2026-06-18, ADR-0014) — ✅ edge half closed for `d3423bc0`.** Ported the constructed student (SigLIP + projector + Qwen2.5-0.5B + LoRA) to MLX in Python and verified it reproduces the PyTorch student: faithful MLX SigLIP (last_hidden_state max|Δ|=2e-04), **assembled-forward parity gate PASSED** (identical greedy output). Apple-Silicon (M4 MLX) perf: **TTFT 118 ms, 80.9 tok/s, peak ≈1.6 GB** — fits the iPhone 16 Pro budget (vs Qwen2.5-VL-3B's non-viable 6.5 GB). This closes the *edge-deployability* half of the proof-of-work (quality was already validated same-path). iPhone Swift port (`MLXVLMRunner`) now **optional confirmation**, not a blocker. Tool: `runners/export_student_mlx.py`. See [observation](docs/observations/2026-06-18-on-device-mlx-validation.md).
