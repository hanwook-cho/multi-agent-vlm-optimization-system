...
- **B1.3 (next)** — first real construction run (generate the balanced cache, real align+distill budgets); wire the assembled student into the same-path VLMEvalKit eval; score vs the LFM2-VL-450M benchmark (POPE ≥ ~86), record real `quality_scores`, agent re-routes on the outcome.
