...
HYPOTHESIS_TABLE = [
    ...
    },
    {
        ...
        "result_summary": (
            ...
            "UPDATE 5 (seeded variance test, 2026-06-17): build_student had NO RNG seeding → the from-scratch "
            "projector init was uncontrolled. Fixed (seed_everything, --seed). Re-ran the 50/50 milestone "
            "(151cf686) across 3 seeds + the original: MMBench ROBUST (0.59-0.70, 4/4 above floor — the "
            "distribution-matching result reproduces), but POPE HIGH-VARIANCE (bal-acc 5-62, only 2/4 above "
            "floor). So 'one student clears BOTH floors' was a favorable POPE draw, not a solved property — "
            "MMBench from matched data is real; holding POPE simultaneously is a coin-flip at 0.5B/50-50. The "
            "two prior lever 'negatives' (ratio 60/40, rank 32) were init variance, NOT lever effects — "
            "discarded. RULE GOING FORWARD: all construction claims (esp. POPE) are multi-seed (median/min "
            "over ≥3 seeds); no single-run milestones. Next: grounding-heavier mix + optimise the MIN POPE "
            "across seeds; capacity if POPE stays unstable. See observation 2026-06-17-p2b1-variance-seeding. "
            "UPDATE 6 (60/40 grounding-heavy, 3 seeds, 2026-06-18): PARTIAL. Shifting 50/50→60/40 lifted POPE "
            "bal-acc median 13.3→48.3 (min 5→30) — more grounding clearly raises POPE — but POPE still only "
            "1/3 seeds above floor (median just under 50), spread still ~40pts; MMBench dropped to 0.55-0.58 "
            "(less ScienceQA). KEY: the ratio shifts POPE's MEAN, not its VARIANCE → the wall is capacity/"
            "training-stability at 0.5B, not data balance. No mix yet gives one student reliably above ≥2 "
...
def _tool_propose_student(
    ...
    distill_rehearse_data: str | None = None,
    distill_rehearse_frac: float = 0.2,
    ...
    try:
        spec = StudentSpec(
            ...
            distill={"teacher": distill_teacher, "data": distill_data,
                     "rehearse_data": distill_rehearse_data,
                     "rehearse_frac": distill_rehearse_frac,
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "distill_rehearse_data": {"type": ["string", "null"], "description": "Optional second cache key replayed in to protect a prior skill from forgetting, e.g. 'mcq' or 'qa_balanced_5k' (continual-learning rehearsal; P2-D1/D2 lesson). Null = no rehearsal."},
                "distill_rehearse_frac": {"type": "number", "description": "Fraction of primary rows' worth of rehearse_data to mix in (0.0-1.0). Only used when distill_rehearse_data is set."},
